...
if (isset($_SESSION['user_id'])) {
    ...
    $sql_user = "SELECT full_name FROM patient_signup WHERE id = '$user_id'";
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div class="doctor-card bg-white dark:bg-surface-dark rounded-[24px] p-6 flex flex-col items-center text-center shadow-sm hover:shadow-lg transition-all border border-transparent hover:border-green-50 dark:hover:border-green-900" data-name="<?php echo strtolower($doc_name); ?>" data-spec="<?php echo strtolower($spec); ?>">
                    ...
                    <div class="w-full space-y-3 mt-auto">
                        <a href="chat1.php?id=<?php echo $row['id']; ?>" class="block w-full py-2.5 bg-primary hover:bg-[var(--primary)] text-white rounded-xl font-semibold transition-colors flex items-center justify-center gap-2"><span class="material-symbols-outlined text-[18px]">chat</span> Chat Now</a>
                        <a href="book.php?id=<?php echo $row['id']; ?>" class="block w-full py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-semibold transition-colors flex items-center justify-center gap-2"><span class="material-symbols-outlined text-[18px]">calendar_month</span> Book</a>
                    </div>
                </div>
                <?php 
                    } 
